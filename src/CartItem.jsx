...
const CartItem = ({ onContinueShopping }) => {
  ...
  const calculateTotalAmount = () => {
    let totalCost = 0;
    cart.forEach(item => {
      const amount = parseFloat(item.cost.substring(1)); // Remove the '$' symbol and convert cost to a number
      totalCost += amount * item.quantity; // Multiply cost by quantity for each item and add to total
    });
